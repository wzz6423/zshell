...
final class AlacrittyTerminalView: NSView, TerminalBackendSurface, NSUserInterfaceValidations {
    ...
    override func mouseDragged(with event: NSEvent) {
        ...
        guard let handle, let selectionAnchor else { return }
        let point = gridPoint(for: event)
        if inputSelectionDragActive {
            moveCursorToClick(point)
            return
        }
        if beginInputSelection(from: selectionAnchor, to: point) {
            return
        }
    ...
    override func mouseUp(with event: NSEvent) {
        ...
        if selectionAnchor != nil, !selectionWasDragged, event.clickCount == 1 {
            moveCursorToClick(gridPoint(for: event))
        }
        stopSelectionAutoscroll()
        selectionAnchor = nil
        selectionWasDragged = false
    ...
    private func moveCursorToClick(
        ...
        guard canMoveCursorToClick(point) else { return }
        ...
        let clickedColumn = min(
            point.column + (point.rightHalf ? 1 : 0), max(gridSize.columns - 1, 0)
        )
        let offset = clickedColumn - Int(snapshot.cursor_column)
        ...
        let key = AlacrittyKeyMap.horizontalCursor(right: offset > 0, mode: mode)
        write(Array(repeating: key, count: abs(offset)).flatMap { $0 })
        scheduleRender(force: true)
    }
    ...
    private func canMoveCursorToClick(
        ...
        guard snapshot.display_offset == 0,
              snapshot.cursor_line >= 0,
              snapshot.cursor_column >= 0,
              point.line == Int(snapshot.cursor_line)
        else { return false }
        return true
    }

    /// A drag inside zsh's current prompt becomes a ZLE region instead of a
    /// terminal-output selection. The marker is ignored by other shells.
    private func beginInputSelection(
        from start: (line: Int, column: Int),
        to end: (line: Int, column: Int, rightHalf: Bool)
    ) -> Bool {
        guard supportsInputSelection,
              start.line == end.line,
              canMoveCursorToClick((start.line, start.column, false)),
              canMoveCursorToClick(end)
        else { return false }

        moveCursorToClick((start.line, start.column, false))
        write(Array("\u{1F}".utf8))
        moveCursorToClick(end)
        selectionWasDragged = true
