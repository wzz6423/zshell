...
final class ZshellTerminalView: AppTerminalView, TerminalBackendSurface {
    ...
    override func mouseDragged(with event: NSEvent) {
        guard supportsInputSelection else {
            super.mouseDragged(with: event)
            return
        }
        if !inputSelectionDragActive {
            // Ghostty moves the shell cursor for the button press; mark that
            // point before its release event moves the cursor to drag's end.
            // A one-byte ZLE marker avoids the ESC-sequence timeout that can
            // occur when Ghostty injects synthetic text character by character.
            sendText("\u{1F}")
